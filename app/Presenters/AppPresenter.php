...
class AppPresenter
{
    public function title($url)
    {
        if (strpos($url, 'register') !== false) {
            return '註冊會員';
        } elseif (strpos($url, 'back/about_us') !== false) {
            return '後台 > 關於我們';
        } elseif (strpos($url, 'back/total') !== false) {
            return '後台 > 足跡管理';
        } elseif (strpos($url, 'back/goods') !== false) {
            return '後台 > 商品管理';
        } elseif (strpos($url, 'back/orders') !== false) {
            return '後台 > 訂單管理';
        } elseif (strpos($url, 'back/users') !== false) {
            return '後台 > 會員管理';
        } elseif (strpos($url, 'back/admins') !== false) {
            return '後台 > 管理員們';
        } elseif (strpos($url, 'back/messages') !== false) {
            return '後台 > 留言管理';
        } elseif (strpos($url, 'back/articles') !== false) {
            return '後台 > 文章管理';
        } elseif (strpos($url, 'back/articles/create') !== false) {
            return '後台 > 文章管理 > 新增';
        } elseif (strpos($url, 'back/articles/edit') !== false) {
            return '後台 > 文章管理 > 編輯';
        } elseif (strpos($url, 'back/articles/editing') !== false) {
            return '後台 > 文章管理 > 編輯中';
        } elseif (strpos($url, 'about_us') !== false) {
            return '關於我們';
        } elseif (strpos($url, 'member') !== false) {
            return '會員中心';
        } elseif (strpos($url, 'articles') !== false) {
            return '文章';
        } elseif (strpos($url, 'cart') !== false) {
            return '購物車';
        } elseif (strpos($url, 'adoption') !== false) {
            return '領養名單';
        } elseif (strpos($url, 'back_login') !== false) {
            return '管理員登入';
        } elseif (strpos($url, 'back') !== false) {
            return '後台';
    ...
    public function fullTitle($url)
    {
        $baseTitle = '奇多喵合作社';
        $specificTitle = $this->title($url);

        if ($specificTitle === $baseTitle) {
            return $baseTitle;
        } else {
            return $baseTitle . ' > ' . $specificTitle;
        }
    }
    ...
    public function captcha($str){
        ...
        for($i=0;$i<mb_strlen($gstr);$i++){
            ...
            $tmp = imagettfbbox($fontsize, $text_info[$char]['angle'], resource_path('fonts/arial.ttf'), $char);
        ...
        foreach($text_info as $char => $info){
            ...
            imagettftext($dst_img,$fontsize,$info['angle'],$x_pointer,$y,$colors[rand(0,count($colors)-1)],resource_path('fonts/arial.ttf'),$char);
    ...
    public function getTotalValue()
{
    ...
    $latestTotal = Total::latest()->first();
    return $latestTotal ? $latestTotal->total : 0;
}
}